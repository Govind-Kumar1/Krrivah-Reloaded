  ...
  useEffect(() => {
    const fetchProjects = async () => {
      ...
      try {
        ...
        // console.log("Fetched Projects Data:", projectsFromApi);
        ...
        const formattedProjects = projectsFromApi.filter((e)=>e.isActive).map((p) => ({
          ...
          title: p.title,
          image: p.images?.[0] || "",
          tags: [p.category, p.location].filter(Boolean), // ✅ Location added in tags
  ...
  return (
    <section className="w-full py-24">
      ...
      <div className="flex justify-between items-center mb-18 px-10">
        ...
        <button onClick={()=>navigate("/projects")} className="text-sm font-semibold border border-gray-400 md:h-[40px] w-[103px] rounded-full text-[#0B2204] px-4 py-2 hover:bg-[#393F36] hover:cursor-pointer hover:text-white">
          VIEW ALL
        </button>
      </div>
              ...
              <div className="flex gap-2 text-xs font-semibold uppercase tracking-wide mb-4 flex-wrap">
                {projects[current].tags.map((tag, idx) => {
                  ...
                  return (
                    ...
                    >
                      {cleanTag.includes("goa") && <LocationIcon />}
                      {tag}
                    </span>
                  );
                })}
